---
title: "Code for Predicted decline in common bird and butterfly species despite conservation policies in Europe"
...
Remark: The complete script should take a few minutes to run. The biodiversity data are the property of the Pan-European Bird Monitoring Programme and the European Butterfly Monitoring Programme and cannot be shared to run this code. A demonstration based on a subsample of anonymised data is provided for both taxa. The external datasets already published for pressures must be downloaded before running the dedicated sections.
...
# Biophysical regions by continuous polygons
grid_eu_mainland_biogeo_cast <- st_read("raw_data/grid_eu_mainland_biogeo_cast.gpkg")
...
Load the scenario value datasets

```{r}

lulc_pls_short <- read.csv("output/lulc_pls_short.csv") # land-use and land-use intensity scenarios
pa_pls_short <- read.csv("output/pa_pls_short.csv") # protected area scenarios
climate_pls <- read.csv("output/climate_pls.csv") # climate scenario

```


#### Re-compute the datasets from orignial data

Alternatively, the scenario value datasets can be recalculated from the original data


##### Land-use and land-use intensity scenarios
...
# download change in protected areas from https://dataverse.nl/dataset.xhtml?persistentId=doi:10.34894/TCNKPJ
...
# download change in climate https://cds.climate.copernicus.eu/

### Temperature according to the different models
...
## Analysis of pressure effects and scenarios
...
res_gamm_bird_ex <- gam_bird_PLS(bird_data, pressure_data = pressure_publi, pressure_data_unscale = pressure_publi_unscale,
                                 lulc_pls_short = lulc_pls_short, climate_pls = climate_pls, pa_pls_short = pa_pls_short)

res_gamm_bird_ex[which(res_gamm_bird_ex$PLS=="europe"),] 

```

In this example, the effects on species abundance (beta coefficients in the related publication) are: positive effects of open environments compared to forest and semi-natural environments on abundance (milieu_catopenland = 1.19\*, milieu_catothers = 2.93\*), a negative effect of urban environments compared to forest and semi-natural environment on abundance (milieu_caturban = - 1.83\*), a negative effect of temperature on abundance (tempspring = -0.39), a positive effect of precipitation on abundance (precspring = 1.53\*), a negative effect of landscape diversity on  abundance (shannon = - 0.30\*) and a negative effect of ecosystem productivity (drymatter = -0.86\*).

The effects on species temporal trend (alpha coefficients in the related publication) are: a negative effect of increase in imperiousness (year:d_impervious = -0.05), a negative effect of increase in tree density (year:d_treedensity = -0.02), negative effects of low/medium and high intensity management forest (year:eulandsystem_forest_lowmedium = -0.10\*, year:eulandsystem_forest_high = -0.02), a positive effect of increase in agricultural cover (year:d_agri = 0.04), a positive effect of low intensity farmland cover (year:eulandsystem_farmland_low = 0.05), a negative effect of high intensity farmland cover (year:eulandsystem_farmland_high = -0.03), a positive effect of temperature increase (year:d_tempsrping = 0.03), a positive effect of temperature variation increase (year:d_tempsrpingvar = 0.03), a negative effect of precipitation increase (year:d_precspring = -0.04), a negative effect of increase in landscape diversity (year:d_shannon = -0.005) and a positive effect of protected area (year:protectedarea_perc = 0.04).
...
# Load results based on all data

res_gamm_bird <- read.csv("output/res_gam_bird.csv")

res_gamm_bird_correct <- res_gamm_bird[which(res_gamm_bird$dev_exp>0.20),]
...
pressure_EU_bird_nosignif_all_species <- res_gamm_bird_correct[which(res_gamm_bird_correct$PLS=="europe" & res_gamm_bird_correct$pressure_removed == "none"),
                                          c("year.d_impervious","year.d_tempsrping","year.d_tempsrpingvar",
                                            "year.d_precspring","year.d_shannon","year.protectedarea_perc",
                                            "year.d_treedensity","year.eulandsystem_forest_lowmedium","year.eulandsystem_forest_high",
...
pressure_EU_bird_all_species <- res_gamm_bird_correct[which(res_gamm_bird_correct$PLS=="europe" & res_gamm_bird_correct$pressure_removed == "none"),
                                                   c("year.d_impervious_signif","year.d_tempsrping_signif","year.d_tempsrpingvar_signif",
                                                     "year.d_precspring_signif","year.d_shannon_signif","year.protectedarea_perc_signif",
                                                     "year.d_treedensity_signif","year.eulandsystem_forest_lowmedium_signif",
                                                     "year.eulandsystem_forest_high_signif","year.d_agri_signif",
                                                     "year.eulandsystem_farmland_low_signif","year.eulandsystem_farmland_high_signif",
                                                     "milieu_catopenland_signif","milieu_caturban_signif","tempsrping_signif",
                                                     "precspring_signif","shannon_signif","drymatter_signif",
                                                     "PLS","dev_exp","n_obs","sci_name_out")]

pressure_EU_bird_nosignif <- pressure_EU_bird_nosignif_all_species
pressure_EU_bird <- pressure_EU_bird_all_species

# farmland species

pressure_EU_bird_nosignif <- pressure_EU_bird_nosignif_all_species[which(pressure_EU_bird_nosignif_all_species$sci_name_out %in% farmland_species),]
pressure_EU_bird <- pressure_EU_bird_all_species[which(pressure_EU_bird_all_species$sci_name_out %in% farmland_species),]

# forest species

pressure_EU_bird_nosignif <- pressure_EU_bird_nosignif_all_species[which(pressure_EU_bird_nosignif_all_species$sci_name_out %in% forest_species),]
pressure_EU_bird <- pressure_EU_bird_all_species[which(pressure_EU_bird_all_species$sci_name_out %in% forest_species),]
...
pressure_EU_bird_long_d <- pressure_EU_bird_long[which(pressure_EU_bird_long$variable %in%
                                                         c("year.d_impervious","year.d_tempsrping","year.d_tempsrpingvar",
                                                           "year.d_precspring","year.d_shannon","year.protectedarea_perc",
                                                           "year.d_treedensity","year.eulandsystem_forest_lowmedium","year.eulandsystem_forest_high",
...
pressure_EU_bird_long_d$variable <- factor(pressure_EU_bird_long_d$variable , 
                                           levels = c("year.eulandsystem_farmland_high","year.eulandsystem_farmland_medium",
                                                      "year.eulandsystem_farmland_low","year.d_agri","year.eulandsystem_forest_high",
                                                      "year.eulandsystem_forest_lowmedium","year.d_treedensity","year.protectedarea_perc",
                                                      "year.d_shannon","year.d_impervious","year.d_precspring","year.d_tempsrpingvar",
                                                      "year.d_tempsrping"))


ggplot(pressure_EU_bird_long_d, aes(x = value_nosignif)) +
...
res_gamm_butterfly_ex <- gam_butterfly_PLS(butterfly_data, pressure_data = pressure_publi_butterfly, pressure_data_unscale = pressure_publi_butterfly_unscale, lulc_pls_short = lulc_pls_short, climate_pls = climate_pls, pa_pls_short = pa_pls_short)

res_gamm_butterfly_ex[which(res_gamm_butterfly_ex$PLS == "europe"),]

```

In this example, the effects on species abundance (beta coefficients in the related publication) are: a positive effect of open environments compared to forest and semi-natural environments on abundance (milieu_catopenland = 0.16), a negative effect of marine and wetland environment compared to forest and semi-natural environments on abundance (milieu_catothers = -0.47), a negative effect of urban environments compared to forest and semi-natural environment on abundance (milieu_caturban = -0.13), a positive effect of temperature on abundance (tempspring = 3.67), a positive effect of precipitation on abundance (precspring = 3.38\*), a negative effect of landscape diversity on  abundance (shannon = -0.06) and a positive effect of ecosystem productivity (drymatter = 0.70\*).

The effects on species temporal trend (alpha coefficients in the related publication) are: a positive effect of increase in imperiousness (year:d_impervious = 0.03), a positive effect of increase in tree density (year:d_treedensity = 0.02), negative effects of low/medium and high intensity management forest (year:eulandsystem_forest_lowmedium = -0.02, year:eulandsystem_forest_high = -0.04), a negative effect of increase in agricultural cover (year:d_agri = -0.01), a positive effect of low intensity farmland cover (year:eulandsystem_farmland_low = 0.09), a negative effect of high intensity farmland cover (year:eulandsystem_farmland_high = -0.05), a negative effect of temperature increase (year:d_tempsrping = -0.03), a negative effect of temperature variation increase (year:d_tempsrpingvar = -0.24\*), a negative effect of precipitation increase (year:d_precspring = -0.28\*), a positive effect of increase in landscape diversity (year:d_shannon = 0.09) and a negative effect of protected area (year:protectedarea_perc = -0.09\*).
...
res_gamm_butterfly <- read.csv("output/res_gam_butterfly.csv")
...
pressure_EU_butterfly_nosignif_all_species <- res_gamm_butterfly_correct[which(res_gamm_butterfly_correct$PLS=="europe" & res_gamm_butterfly_correct$pressure_removed == "none"),
                                          c("year.d_impervious","year.d_tempsrping","year.d_tempsrpingvar",
                                            "year.d_precspring","year.d_shannon","year.protectedarea_perc",
                                            "year.d_treedensity","year.eulandsystem_forest_lowmedium","year.eulandsystem_forest_high",
                                            "year.d_agri","year.eulandsystem_farmland_low","year.eulandsystem_farmland_high",
                                            "milieu_catopenland","milieu_caturban","tempsrping",
                                            "precspring","shannon","drymatter",
                                            "PLS","dev_exp","n_obs","species_name")]
pressure_EU_butterfly_all_species <- res_gamm_butterfly_correct[which(res_gamm_butterfly_correct$PLS=="europe" & res_gamm_butterfly_correct$pressure_removed == "none"),
                                                   c("year.d_impervious_signif","year.d_tempsrping_signif","year.d_tempsrpingvar_signif",
                                                     "year.d_precspring_signif","year.d_shannon_signif","year.protectedarea_perc_signif",
                                                     "year.d_treedensity_signif","year.eulandsystem_forest_lowmedium_signif",
                                                     "year.eulandsystem_forest_high_signif","year.d_agri_signif",
...
pressure_EU_butterfly_nosignif <- pressure_EU_butterfly_nosignif_all_species
pressure_EU_butterfly <- pressure_EU_butterfly_all_species

# grassland species

pressure_EU_butterfly_nosignif <- pressure_EU_butterfly_nosignif_all_species[which(pressure_EU_butterfly_nosignif_all_species$species_name %in% grassland_species),]
pressure_EU_butterfly <- pressure_EU_butterfly_all_species[which(pressure_EU_butterfly_all_species$species_name %in% grassland_species),]

# woodland species

pressure_EU_butterfly_nosignif <- pressure_EU_butterfly_nosignif_all_species[which(pressure_EU_butterfly_nosignif_all_species$species_name %in% woodland_ind_species),]
pressure_EU_butterfly <- pressure_EU_butterfly_all_species[which(pressure_EU_butterfly_all_species$species_name %in% woodland_ind_species),]
...
pressure_EU_butterfly_long_nosignif <- reshape::melt(pressure_EU_butterfly_nosignif, id.vars=c("species_name","PLS"))
pressure_EU_butterfly_long_nosignif <- pressure_EU_butterfly_long_nosignif[which(!pressure_EU_butterfly_long_nosignif$variable %in% c("(Intercept)","PLS","dev_exp","n_obs")),]


pressure_EU_butterfly_long$value_nosignif <- pressure_EU_butterfly_long_nosignif$value
pressure_EU_butterfly_long$signif <- as.factor(ifelse(is.na(pressure_EU_butterfly_long$value), 0,1))
pressure_EU_butterfly_long$variable_nosignif <- pressure_EU_butterfly_long$variable 
pressure_EU_butterfly_long$variable_nosignif[which(pressure_EU_butterfly_long$signif==0)] <- NA
pressure_EU_butterfly_long$variable_nosignif <- gsub(pattern = "_signif","",pressure_EU_butterfly_long$variable_nosignif)
pressure_EU_butterfly_long$variable <- gsub(pattern = "_signif","",pressure_EU_butterfly_long$variable)

pressure_EU_butterfly_long_d <- pressure_EU_butterfly_long[which(pressure_EU_butterfly_long$variable %in%
                                                         c("year.d_impervious","year.d_tempsrping","year.d_tempsrpingvar",
                                                           "year.d_precspring","year.d_shannon","year.protectedarea_perc",
                                                           "year.d_treedensity","year.eulandsystem_forest_lowmedium","year.eulandsystem_forest_high",
                                                           "year.d_agri","year.eulandsystem_farmland_low","year.eulandsystem_farmland_medium",
...
pressure_EU_butterfly_long_d$variable <- factor(pressure_EU_butterfly_long_d$variable , 
                                           levels = c("year.eulandsystem_farmland_high","year.eulandsystem_farmland_medium",
                                                      "year.eulandsystem_farmland_low","year.d_agri","year.eulandsystem_forest_high",
                                                      "year.eulandsystem_forest_lowmedium","year.d_treedensity","year.protectedarea_perc",
                                                      "year.d_shannon","year.d_impervious","year.d_precspring","year.d_tempsrpingvar",
...
#### Birds by biophysical region

```{r}

# selected species used in PECBMS indicators and with good-enough model fit
...
"Cuculus canorus",
"Sylvia cantillans / Sylvia subalpina","Sylvia communis",
"Sylvia curruca","Sylvia hortensis","Sylvia melanocephala",
"Sylvia nisoria","Sylvia undata",
"Curruca melanothorax","Cyanistes caeruleus",
...
"Cyanecula svecica","Lyrurus tetrix",
...
predict_trend_all_bird_correct <- res_gamm_bird[which(res_gamm_bird$sci_name_out %in% pecbms_species & res_gamm_bird$dev_exp > 0.2),]

# Bound the predicted trends within values of observed past trends

predict_trend_all_bird_correct <- ddply(predict_trend_all_bird_correct,
                                             .(PLS),.fun=function(x){
                                               for(i in c("trend_past","trend_BAU","trend_SSP1","trend_SSP3","trend_nac","trend_nfn","trend_nfs","trend_past_signif","trend_BAU_signif","trend_SSP1_signif","trend_SSP3_signif","trend_nac_signif","trend_nfn_signif","trend_nfs_signif")){
...
overall_trend_all_sf <- merge(grid_eu_mainland_biogeo,
                              overall_trend_farmland_bird, # or overall_trend_all_bird, overall_trend_forest_bird
                              by="PLS",all.x=TRUE)

ggplot() + geom_sf() +  
  geom_sf(data=overall_trend_all_sf[which(overall_trend_all_sf$n>=3),], aes(fill=mu_past), col = NA) + 
  geom_sf(data=overall_trend_all_sf[which(is.na(overall_trend_all_sf$n)),], fill="grey50", col = NA) + 
  geom_sf(data=overall_trend_all_sf[which(overall_trend_all_sf$n<3),], fill="grey50", col = NA) + 
...
#### Butterflies by biophysical region

```{r}

# selected species with good-enough model fit

predict_trend_all_butterfly_correct <- res_gamm_butterfly[which(res_gamm_butterfly$dev_exp > 0.2),]
...
overall_trend_all_sf <- merge(grid_eu_mainland_biogeo_cast,
                              overall_trend_grassland_butterfly, # or overall_trend_all_butterfly, overall_trend_woodland_butterfly
                              by="PLS",all.x=TRUE)

ggplot() + geom_sf() +  
  geom_sf(data=overall_trend_all_sf[which(overall_trend_all_sf$point>0 & overall_trend_all_sf$n>=3),], aes(fill=mu_past), col = NA) + 
  geom_sf(data=overall_trend_all_sf[which(overall_trend_all_sf$point==0 | overall_trend_all_sf$n < 3 | is.na(overall_trend_all_sf$n)),], fill="grey50", col = NA) + 
  geom_sf(data=grid_eu_mainland_outline, fill=NA) +
  scale_fill_gradient2(midpoint = 1, name = NULL) + theme_void()
...
#### Birds and butterflies at the Euroepan level
...
europe_all <- data.frame(value = c(overall_trend$mu_past[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                          overall_trend$mu_bau[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                          overall_trend$mu_ssp1[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                          overall_trend$mu_nac[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                          overall_trend$mu_nfn[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                          overall_trend$mu_nfs[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)]),
                                sd = c(overall_trend$sd_past[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                       overall_trend$sd_bau[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                       overall_trend$sd_ssp1[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                       overall_trend$sd_nac[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                       overall_trend$sd_nfn[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                       overall_trend$sd_nfs[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)]),
                                se = c(overall_trend$se_past[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                       overall_trend$se_bau[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                       overall_trend$se_ssp1[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                       overall_trend$se_nac[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                       overall_trend$se_nfn[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                       overall_trend$se_nfs[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)]),
                                variable = c("past","bau","ssp1","nac","nfn","nfs"))

europe_all2 <- europe_all
...
ggplot(europe_all2, aes(x=value,y = variable)) + 
  ...
  scale_color_manual(values = c("past"="black","bau"="grey","ssp1"="#cc79a7ff","nfn"="#009e73ff","nfs"="#0072b2ff","nac"="#e69f00ff")) + 
...
ggplot(data.frame(x = 2000:2050), aes(x)) +
  geom_function(fun = function(x){europe_all$value[which(europe_all$variable=="past")]^x/europe_all$value[which(europe_all$variable=="past")]^2021*100}, colour = "black", linetype=1, xlim=c(2000,2021)) +
  geom_function(fun = function(x){europe_all$value[which(europe_all$variable=="bau")]^x/europe_all$value[which(europe_all$variable=="bau")]^2021*100}, colour = "grey", linetype=2, xlim=c(2021,2050)) + 
  geom_function(fun = function(x){europe_all$value[which(europe_all$variable=="ssp1")]^x/europe_all$value[which(europe_all$variable=="ssp1")]^2021*100}, colour = "#cc79a7ff", linetype=3, xlim=c(2021,2050)) + 
  geom_function(fun = function(x){europe_all$value[which(europe_all$variable=="nfn")]^x/europe_all$value[which(europe_all$variable=="nfn")]^2021*100}, colour = "#009e73ff", linetype=4, xlim=c(2021,2050)) + 
  geom_function(fun = function(x){europe_all$value[which(europe_all$variable=="nfs")]^x/europe_all$value[which(europe_all$variable=="nfs")]^2021*100}, colour = "#0072b2ff", linetype=5, xlim=c(2021,2050)) + 
  geom_function(fun = function(x){europe_all$value[which(europe_all$variable=="nac")]^x/europe_all$value[which(europe_all$variable=="nac")]^2021*100}, colour = "#e69f00ff", linetype=6, xlim=c(2021,2050)) + 
  coord_trans(y='log') + ylim(c(25,200)) + 
  theme_minimal() + xlab("Year") + ylab("Abundance")

# Pair-wise difference between scenarios

europe_all2$variable <- as.character(europe_all2$variable)
comb_var <- combn(europe_all2$variable,2)
test_diff_var_europe_all <- data.frame(cbind(t(comb_var),NA))
for(i in 1:dim(comb_var)[2]){
  test_diff_var_europe_all[i,3] <- tsum.test(mean.x=europe_all2$value[which(europe_all2$variable==comb_var[1,i])],   s.x=europe_all2$se[which(europe_all2$variable==comb_var[1,i])], n.x= overall_trend$n[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)],
                                                    mean.y=europe_all2$value[which(europe_all2$variable==comb_var[2,i])],   s.y=europe_all2$se[which(europe_all2$variable==comb_var[2,i])], n.y= overall_trend$n[which(overall_trend$PLS=="europe" & overall_trend$pressure_removed == pressure_removed)])$p.value
  
}

test_diff_var_europe_all
